...
interface DashboardData {
  ...
  revenue:   { label: string; amount: string }[];
}

// ── useDashboard hook ─────────────────────────────────────────────────────────
// Fetches live data when a real API is available; falls back to mock data.
// To wire up a real backend, set VITE_DASHBOARD_API in your .env:
//   VITE_DASHBOARD_API=https://your-api.vercel.app
// The endpoint should return JSON matching DashboardData.
// Until then, mock data is used automatically.
...
function useDashboard(): { data: DashboardData; loading: boolean; refresh: () => void } {
  const [data, setData]       = useState<DashboardData>(MOCK);
  const [loading, setLoading] = useState(false);

  const load = useCallback(async () => {
    const base = import.meta.env.VITE_DASHBOARD_API;
    if (!base) return; // no env var → stay on mock data, no request fired
    ...
    try {
      ...
      console.warn("[GeneralIntelligence] Dashboard API unavailable, using mock data.", err);
      setData(MOCK);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    load();
    // Refresh every 60 seconds when a real API is configured
    const interval = import.meta.env.VITE_DASHBOARD_API
...
// ── AI advisor ────────────────────────────────────────────────────────────────
// Calls through your existing /api/claude proxy — keeps the Anthropic key server-side.
...
async function askAdvisor(messages: ChatMessage[], userMsg: string): Promise<string> {
...
const css = `
  ...
  .gi-dot { width:6px; height:6px; border-radius:50%; background:#4ade80; animation:gi-pulse 2s infinite; }
  .gi-dot.loading { background:#d4af37; }
  ...
  .gi-milestone-dot.active  { background:#d4af37; animation:gi-pulse 1.5s infinite; }
  ...
  .gi-data-note { font-family:'JetBrains Mono',monospace; font-size:0.44rem; color:rgba(138,130,120,0.3); letter-spacing:0.1em; margin-top:0.5rem; }
`;
...
export default function GeneralIntelligence() {
  ...
  const isLive = !!import.meta.env.VITE_DASHBOARD_API;
  ...
  const filteredMilestones = activeTrack === "all"
    ? data.milestones
    : data.milestones.filter(m => m.track === activeTrack);
  ...
  const ask = async (msg: string) => {
    ...
    setBusy(true);
    try {
      const reply = await askAdvisor(chat, userMsg);
    ...
    <div className="gi-root">
      ...
      <div className="gi-topbar">
        ...
        <div className="gi-status">
          <div className={`gi-dot${loading ? " loading" : ""}`} />
          <span>{isLive ? "Live data" : "Mock data"} · Q1 2026 · AUM Target: $2–5M</span>
          {isLive && (
            <button onClick={refresh} style={{ background:"none", border:"none", color:"rgba(212,175,55,0.3)", cursor:"pointer", fontFamily:"'Cinzel',serif", fontSize:"0.44rem", letterSpacing:"0.1em", marginLeft:"0.5rem" }}>
              ↻ Refresh
            </button>
          )}
        </div>
      ...
      <div className="gi-grid">
        ...
        <div className="gi-panel gi-panel-wide">
          <div className="gi-panel-label">
            ...
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.5rem", color: isLive ? "rgba(74,222,128,0.4)" : "rgba(212,175,55,0.25)" }}>
              {isLive ? "LIVE" : "MOCK"}
            </span>
          </div>
          ...
          {!isLive && (
            <p className="gi-data-note">
              ↳ Add VITE_DASHBOARD_API to .env to connect live data. Mock data shown.
            </p>
          )}
        </div>
        ...
        <div className="gi-panel gi-panel-wide">
          <div className="gi-panel-label">
            ...
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.5rem", color:"rgba(212,175,55,0.3)" }}>
              {data.pipeline.length} prospects
            </span>
          </div>
        ...
        <div className="gi-panel gi-panel-wide">
          <div className="gi-panel-label">
            ...
            <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"0.5rem", color:"rgba(212,175,55,0.3)" }}>
              Claude Sonnet · /api/claude proxy
            </span>
          </div>
        ...
        <div className="gi-panel gi-panel-full">
          <div className="gi-panel-label">
            ...
            <div style={{ display:"flex", gap:"0.5rem" }}>
              {(["all","facinations","xervault"] as const).map(track => (
                <button key={track} onClick={() => setActiveTrack(track)}
                  style={{ background: activeTrack===track ? "rgba(212,175,55,0.1)" : "none", border:`1px solid ${activeTrack===track ? "rgba(212,175,55,0.4)" : "rgba(212,175,55,0.1)"}`, color: activeTrack===track ? "#d4af37" : "rgba(138,130,120,0.4)", fontFamily:"'Cinzel',serif", fontSize:"0.44rem", letterSpacing:"0.15em", textTransform:"uppercase", padding:"0.2rem 0.6rem", cursor:"pointer", borderRadius:"1px" }}>
                  {track === "all" ? "All" : track === "facinations" ? "Facinations" : "XerVault"}
                </button>
              ))}
            </div>
          </div>
